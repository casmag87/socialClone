...
public class JWTAuthenticationFilter implements ContainerRequestFilter {
    ...
    public void filter(ContainerRequestContext request) throws IOException {
        if (isSecuredResource()) {
            ...
            if (token == null) {
                request.abortWith(error(403, "Not authenticated - do login"));
                return;
            }
            ...
            } catch (AuthenticationFailedException | ParseException | JOSEException ex) {
                Logger.getLogger(JWTAuthenticationFilter.class.getName()).log(Level.SEVERE, null, ex);
                request.abortWith(error(403,"Token not valid (timed out?)"));
            }
        }
    }
    ...
    private UserPrincipal getUserPrincipalFromTokenIfValid(String token)
            throws ParseException, JOSEException, AuthenticationFailedException {
        ...
        if (signedJWT.verify(verifier)) {
            if (new Date().getTime() > signedJWT.getJWTClaimsSet().getExpirationTime().getTime()) {
                throw new AuthenticationFailedException("Your Token is no longer valid");
            }
